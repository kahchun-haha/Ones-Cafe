async function fetchFeedback() {
  try {
    ...
    displayFeedback("issues", issues, true);
    ...
    const totalFeedbackCount =
      contacts.length + suggestions.length + issues.length + reviews.length;
...
function displayFeedback(type, data, isIssue = false) {
  ...
  data.forEach((item) => {
    const content =
      type === "contacts"
        ? ""
        : item.content || item.request || item.experiencing || item.ratings;
